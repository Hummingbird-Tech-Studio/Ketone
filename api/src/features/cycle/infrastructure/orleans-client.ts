  ...
  effect: Effect.gen(function* () {
    ...
    return {
        ...
        Effect.gen(function* () {
          ...
          const httpResponse = yield* httpClient.execute(request).pipe(
            Effect.mapError((error) => {
              console.error('[Orleans Client] Connection error:', error);
              return new OrleansClientError({
          ...
          const state = yield* HttpClientResponse.schemaBodyJson(OrleansActorStateSchema)(httpResponse).pipe(
            Effect.mapError((error) => {
              console.error('[Orleans Client] Failed to decode response:', error);
              return new OrleansClientError({
          ...
          yield* Effect.logInfo(`[Orleans Client] ✅ Actor state retrieved successfully`);
          console.log('[Orleans Client] State:', state);
      ...
      persistActor: (actorId: string, state: OrleansActorState) =>
        Effect.gen(function* () {
          yield* Effect.logInfo(`[Orleans Client] POST ${ORLEANS_BASE_URL}/actors/${actorId}`);
          console.log('[Orleans Client] Request body:', JSON.stringify(state, null, 2));
          ...
          if (httpResponse.status !== 200 && httpResponse.status !== 201) {
            yield* Effect.logError(`[Orleans Client] Failed to persist: HTTP ${httpResponse.status}`);
            return yield* Effect.fail(
              new OrleansClientError({
          ...
          yield* Effect.logInfo(`[Orleans Client] ✅ Actor ${actorId} state persisted to Orleans`);
        }),
    };
  }),
  accessors: true,
