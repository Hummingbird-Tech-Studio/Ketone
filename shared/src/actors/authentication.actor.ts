...
/**
 * Storage Programs Interface
 * Platform-specific implementations should provide these programs
 */
export type StoragePrograms = {
  programCheckSession: Effect.Effect<AuthSession | null, unknown>;
  programStoreSession: (session: AuthSession) => Effect.Effect<void, unknown>;
  programRemoveSession: Effect.Effect<void, unknown>;
};
...
/**
 * Factory to create authentication machine with platform-specific storage programs
 */
export const createAuthenticationMachine = (storage: StoragePrograms) =>
  setup({
    ...
    actors: {
      checkAuthSession: fromCallback<EventObject>(({ sendBack }) =>
        runWithUi(
      ...
      storeAuthSession: fromCallback<EventType, { token: string; user: UserResponseSchema }>(({ sendBack, input }) =>
        runWithUi(
          ...
          (error) => {
            console.error('[Auth Actor] Failed to store session:', error);
            sendBack({
      ...
      removeAuthSession: fromCallback<EventObject>(({ sendBack }) =>
        runWithUi(
          ...
          (error) => {
            console.error('[Auth Actor] Failed to remove session:', error);
            sendBack({ type: Event.DEAUTH_COMPLETE });
          },
        ),
      ),
...
export type AuthenticationActor = ActorRefFrom<AuthenticationMachine>;
